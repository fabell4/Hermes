    ...
    <div className="min-h-screen bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-100">
      ...
      <header className="fixed top-0 left-0 right-0 z-40 h-20 border-b border-slate-200 dark:border-slate-800 bg-white/90 dark:bg-slate-950/90 backdrop-blur flex items-center px-4 gap-3">
        ...
        <div className="flex items-center gap-2.5">
          ...
          {currentVersion && (
            <span className="hidden sm:inline text-xs px-2 py-0.5 rounded-full bg-slate-100/60 dark:bg-slate-800/60 text-slate-500 border border-slate-200/50 dark:border-slate-700/50 font-mono">
              v{currentVersion}
            </span>
          )}
          {updateAvailable && (
        ...
        <button
          onClick={toggleTheme}
          className="p-2.5 rounded-md text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800/50 transition-colors"
        ...
        {mobileOpen && (
          <>
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="fixed inset-0 z-30 bg-black/20 dark:bg-slate-950/70 lg:hidden"
              onClick={() => setMobileOpen(false)}
            />
            <motion.div
              initial={{ x: '-100%' }}
