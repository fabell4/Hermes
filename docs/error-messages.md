...
**Level:** ERROR  
**Source:** [src/providers/ookla.py](../src/providers/ookla.py)  
**Cause:** The Ookla `speedtest` binary is not installed or is not in `PATH`  
**Troubleshooting:**

1. Install from <https://www.speedtest.net/apps/cli>
2. Verify: `speedtest --version` (should print "Speedtest by Ookla")
3. If using only `ndt7` or `custom` providers, the Ookla CLI is not required — remove `ookla` from `SPEEDTEST_PROVIDERS`
...
1. Verify CLI is installed: `speedtest --version`
2. Check internet connectivity
3. Test manually: `speedtest --format=json`
...
2. Pin to a faster server via `SPEEDTEST_SERVER_ID`
3. Verify sufficient bandwidth available
4. Check for ISP throttling
5. Consider adding `ndt7` as a fallback: `SPEEDTEST_PROVIDERS=ookla,ndt7`
...
**Level:** ERROR  
**Source:** [src/providers/ndt7.py](../src/providers/ndt7.py)  
**Cause:** M-Lab Locate API unreachable, or WebSocket connection dropped  
**Troubleshooting:**

1. Confirm outbound HTTPS is allowed (Locate API at `locate.measurementlab.net`)
2. Confirm outbound WSS (port 443) is not blocked by firewall
3. M-Lab may be temporarily at capacity — check <https://www.measurementlab.net/status/>
...
**Level:** ERROR  
**Source:** [src/providers/custom_http.py](../src/providers/custom_http.py)  
**Cause:** Request to the configured download or upload URL failed  
**Troubleshooting:**

1. Verify `SPEEDTEST_CUSTOM_URL_DOWNLOAD` is reachable from the container
2. Confirm the URL scheme is `http://` or `https://` (other schemes are rejected for security)
3. For upload: verify the upload endpoint accepts `POST` with `application/octet-stream`
4. Check firewall rules between the Hermes container and the custom endpoint
...
1. Review per-provider error entries above for root cause
2. Check consecutive failure count for alerting
3. Verify internet connectivity is stable
4. Add a resilient fallback, e.g., `SPEEDTEST_PROVIDERS=ookla,ndt7`

**Note:** The primary provider is retried once on failure. Subsequent providers in the chain are each attempted once.
The error from the last failed provider is surfaced.
